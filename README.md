...
- **RPC 调用**：`call<T>` / `call_no_args<T>` 调用 WAAPI 方法，泛型 `T` 为返回值反序列化类型（需 `Serialize + DeserializeOwned`），返回 `Result<Option<T>, Error>`
- **URI 常量**：`waapi-rs::ak` 下提供与 WAAPI 路径对应的嵌套模块与常量（如 `ak::wwise::core::GET_INFO`、`ak::wwise::waapi::GET_TOPICS`），避免手写字符串
...
引入方式只需 `use waapi-rs::ak`，调用时从 `ak::` 开始写路径，与 WAAPI/C++ 的 URI 层级一致（如 `ak.wwise.core.getInfo` 对应 `ak::wwise::core::GET_INFO`）：
...
- `call<T>(uri, args, options)`、`call_no_args<T>(uri)` 的泛型 `T` 为**返回值**的反序列化类型，需满足 `Serialize + DeserializeOwned`（如 `serde_json::Value` 或自定义结构体）。
