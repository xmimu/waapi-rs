...
- **RPC 调用**：`call(uri, args, options)` 调用 WAAPI 方法，返回 `Result<Option<Value>, Error>`，其中 `Value` 为 `serde_json::Value`
- **URI 常量**：`waapi_rs::ak` 下提供与 WAAPI 路径对应的嵌套模块与常量（如 `ak::wwise::core::GET_INFO`、`ak::wwise::waapi::GET_TOPICS`），避免手写字符串
- **主题订阅**：`subscribe(topic, options, callback)` 绑定回调接收事件；通过 `SubscriptionHandle` / `SubscriptionHandleSync` 取消订阅，drop 时自动清理
...
- `call(uri, args, options)` 返回 `Result<Option<Value>, Error>`：成功时 WAAPI 的 kwargs 以 `serde_json::Value` 返回，无结果时为 `None`。
- `args` 与 `options` 为 `Option<serde_json::Value>`（如 `Some(json!({...}))` 或 `None`）。
